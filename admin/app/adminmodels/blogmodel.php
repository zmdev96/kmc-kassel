...
class BlogModel extends AbstractModel
{
    ...
    public static function getBlogFromUserID($userid)
    {
      ...
      $sql = 'SELECT * FROM ' . self::$tableName . ' WHERE UserId = "' . $userid . '"';
        ...
        if(false !== $blogs) {
            foreach ($blogs as $blog) {
                $extractedblogs[] = $blog->BlogId;
            }
        }
        return $extractedblogs;
    }

    public static function blogExists($title, $id = null)
    {
        if (null !== $id) {
          return self::get('
              SELECT * FROM ' . self::$tableName . ' WHERE Title = "' . $title . '" AND BlogId != "' .  $id . '"
          ');
        }else {
          return self::get('
              SELECT * FROM ' . self::$tableName . ' WHERE Title = "' . $title . '"
          ');
        }
    }
    ...
    {
      return self::get('
        SELECT * FROM ' . self::$tableName . ' WHERE Status != 0   ORDER BY BlogId DESC
      ');
