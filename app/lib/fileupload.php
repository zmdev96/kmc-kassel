...
class FileUpload
{
    ...
    private $allowedExtensions = [
        'jpg', 'png', 'gif', 'jpeg','pdf', 'doc', 'docx', 'xls'
    ];

    public function __construct(array $file , $path = null)
    {
        $this->name       = $file['name'];
        $this->type       = $file['type'];
        $this->size       = $file['size'];
    ...
    public function upload()
    {
        if($this->error != 0) {
            ...
            $storageFolder = $this->isImage() ? IMAGES_UPLOAD_STORAGE . $this->customPath : DOCUMENTS_UPLOAD_STORAGE;
