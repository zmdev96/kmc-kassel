...
class AbstractModel
{
    ...
    const DATA_TYPE_CHAR        = \PDO::PARAM_STR_CHAR  ;
    const DATA_TYPE_INT         = \PDO::PARAM_INT;
    const DATA_TYPE_DECIMAL     = 4;
    const DATA_TYPE_DATE        = 5;
    const DATA_TYPE_ALL         ='/^[a-zA-ZÜüÄäÖöß.!,-:?€+$&<>@#""\'=^;:\/\p{Arabic}0-9 ]+$/';
    ...
    public static function getCount($item, $where = null)
    {
      if (null !== $where) {
        $sql = 'SELECT COUNT('. $item .') FROM ' . static::$tableName. ' WHERE '.$where.' = "0"' ;
        $stmt = DatabaseHandler::factory()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchColumn();
      }else{
        $sql = 'SELECT COUNT('. $item .') FROM ' . static::$tableName ;
        $stmt = DatabaseHandler::factory()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchColumn();
      }
